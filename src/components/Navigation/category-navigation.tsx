...
const CategoryNavigation = () => {
  ...
  const allCategories = data?.map(({ name, id }: ResponseName) => (
    <CategoryText key={id} category={name} categoryId={id} />
  ));

  return (
    <div className="flex gap-y-4 gap-x-6 items-center w-full flex-wrap">
      <a
        className="space-x-2 flex items-center cursor-pointer hover:text-red-400 hover:fill-red-400"
        onClick={onHandleAllProducts}
      >
      ...
      <a className="space-x-2 flex items-center cursor-pointer">
        <img
          className="h-4"
          src="https://resources.joomcdn.net/icon/feed@2x.png"
          alt="icon opinions"
        />
        <p className="text-sm font-medium whitespace-nowrap text-red-400">
          Users' reviews
        </p>
      </a>
      {allCategories}
