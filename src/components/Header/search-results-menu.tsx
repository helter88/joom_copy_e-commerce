...
const SearchResultsMenu: React.FC<MenuType> = ({ inputText, setInputText }) => {
  const { isOpen, onCLickHandler, foundProducts } = useSearchResultsMenu(
    inputText,
    setInputText
  );
    ...
    if (titleLower.includes(inputTextLower)) {
      ...
      const boldedLetters2 = (
        <>
          ...
          <span className="font-medium text-black">{`${title.slice(
            startBold,
            endBold
          )}`}</span>
      ...
      const boldedLetters4 = (
        <>
          <span className="text-xs">{`${description.slice(
            0,
            startBold
          )}`}</span>
          <span className="text-xs text-black font-bold">{`${description.slice(
            startBold,
            endBold
          )}`}</span>
